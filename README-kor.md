...
## 4. 개발자 가이드
1. README-prereq에 있는 필수도구 설치
https://github.com/coreutils/coreutils/blob/master/README-prereq

2. README-hacking에 안내된대로 진행
https://github.com/coreutils/coreutils/blob/master/README-hacking

3. 수정된 3가지 파일 교체  
 [참고](https://github.com/YejinHwang-D/rm-execpt-files/tree/main/coreutils_rm/src)  
remove.h  
remove.c  
rm.c  
...
명령어 옵션 테스트
```
~/coreutils/rm -rb [디렉터리명]
~/coreutils/rm -r --except-files [디렉터리명]
```
...
2️⃣  터미널 창에 rm -rb [디렉터리명] 혹은 rm [디렉터리명] -r --execpt-files 옵션을 입력한다.

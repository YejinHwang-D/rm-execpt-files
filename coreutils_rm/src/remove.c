	...
	struct stat* buf)
{
	...
	{
		// mine: 파일 사용자 권한 확인하는 함수 faccessat(). access()와 동일하게 작동.
		if (faccessat(fd_cwd, file, W_OK, AT_EACCESS) == 0)
			// mine: AT_EACCESS: 유효한 사용자 및 그룹 ID를 ㅏ용하여 액세스 확인 수행. 성공하면 0을 반환.
			return 0;

		return errno == EACCES ? 1 : -1;
		// mine: EACCES 권한 거부 메시지
	}
}
			...
			int check_file = 0; //배열을 늘리기 위한 임시 변수
            		...
            		while (true) {
                		...
                		if (fscanf(stdin, "%s", user_file) == 1) {
                			if (user_file == check_command1 || user_file == check_command2)
                				break;
					fprintf(stdout, "\n** 입력된 파일: %s\n\n", user_file);
